...
sg.theme('Black')
def TableSimulation():
	sg.popup_quick_message('Creating Grid', auto_close=True, non_blocking=True, font='Default 18')
	...
	columm_layout =  [[sg.Text(str(i), size=(4, 1), justification='right')] + [sg.InputText(size=(10, 1), pad=(1,1),border_width=0, justification='center', key=str((i, j))) for j in range(MAX_COL)] for i in range(MAX_ROWS)]
	layout = [[sg.Text('Coordinate Grid', font='Any 18')],[sg.Text('Enter the X and Y Coordinates'),],[sg.Col(columm_layout, size=(800, 600), scrollable=True,)]]
	...
	while True:
		...
		for x in range(MAX_COL):
			for g in range(MAX_ROWS):
				location = (g, x)
				target_element = window[str((g, x))]
				if values[str((g, x))] not in ["",None]:
					dctwindow.append({"X":str(location)[1:-1].split(",")[0],"Y":str(location)[1:-1].split(",")[1],"Value":f"{values[str((g, x))]}"})
	window.close()
...
def main():
	...
	for x in list(replaceable.keys()):
		indx = list(replaceable.keys()).index(x)
		try:
