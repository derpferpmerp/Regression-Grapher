...
def quad_regress(xl,yl,rreturn=False,graphing=False):
	...
	try:
		...
		b_eq = (((x3 * x3) * (y1 - y2)) + ((x1 * x1) * (y2 - y3)) + ((x2 * x2) * (y3 - y1))) / bottom
		c_eq = ((x3) * (x2 * (x2 - x3) * y1 + (x1) * (x3 - x1) * y2) + (x1) * (x1 - x2) * (x2 * y3)) / bottom
...
def sig_regress(xl,yl,normalized=True,rreturn=False,graphing=False):
	...
	n = len(xl)
	slope_top = \
		(n * (sm(1, n, f"({xl}[@-1])*({yl}[@-1])"))) \
		- sm(1,n,f"{xl}[@-1]",fval=1) \
		* sm(1,n,f"{yl}[@-1]",fval=1)
	slope_bottom = \
		(n * (sm(1, n, f"({xl}[@-1]**2)",fval=1))) \
		- (sm(1,n,f"{xl}[@-1]"))**2
	slope_sig = slope_top / slope_bottom
	yint = \
		(1/n) * sm(1,n,f"{yl}[@-1]",fval=1) \
	...
	if rreturn:
		original = yl
		predicted = [[math.exp(B0 + (B1 * itr)) * (1 / (1 + math.exp(B0 + (B1 * itr))))][0] for itr in yl]
		return [original,predicted]
	elif graphing:
		original = np.linspace(min(xlist), max(xlist), max(ylist))
...
def main(METHOD, xl, yl):
	if METHOD == "log":
		sg.Popup(str(log_regress(xl,yl)), title = "Logarithmic Output")
	elif METHOD == "lin":
		sg.Popup(str(lin_regress(xl,yl)), title = "Linear Output")
	elif METHOD == "quad":
		sg.Popup(str(quad_regress(xl,yl)), title = "Quadratic Output")
	elif METHOD == "sig":
		sg.Popup(str(sig_regress(xl,yl)), title = "Logistic Output")
	elif METHOD == "all":
		dctout = {}
		...
		plt.legend()
		axes = plt.gca()
		axes.set_ylim([min(ylist) - (0.1 * max(ylist)),max(ylist) + (0.1 * max(ylist))])
		plt.show(block=False)

		for x in lstvalid:
			if x not in list(dctout.keys()): continue
			tmp1, tmp2 = [[],[]]
...
while True:
	...
	if event == 'Exit':
		...
		if not hasclickedbutton and currentbutton == None:
			...
			if set2rand:
				xlist,ylist = genrnd(int(values["-POINTS SLIDER-"]),int(values["-MAXRND SLIDER-"]))
			elif sc != False:
				xlist, ylist = sc
